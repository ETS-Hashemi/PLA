...
| `fidelity_chained_synthetic.*` | 20 seeded three-step rule programs, brute-force deletion oracle | The fallible-ranking test (`scripts/run_fidelity_chained.py`) |
...
- **Fidelity (rule level, the confound-free instrument):** in the flat
  designs both rankings are provably the exact deletion ranking
  (noisy-OR: s − s₋ᵢ = pᵢ·Π(1−pⱼ), monotone in pᵢ within a fired set;
  learned contribution = z_r, unit-tested), so the deletion tables are
  implementation-concordance checks — passed on all four datasets with
  paired ΔCIs excluding zero; fact-level numbers run ~7–10% higher —
  the overlapping-antecedent inflation the rule level removes.
- **Chained programs (the fallible case):** across 20 seeded
  three-step programs, the local candidate ranking picks the true most
  load-bearing rule 32.4% of the time (tie-aware chance 25.8%),
  captures ~49% of the oracle deletion impact, and its margin over a
  random pick flips sign across programs (CI above zero in 9/20, below
  in 7/20) — the load-bearing rule is typically an upstream
  non-redundant one that local magnitude doesn't identify. For chained
  conclusions, trust the leave-one-rule-out counterfactual the engine
  exports, not raw candidate order.
