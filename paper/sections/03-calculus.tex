...
\texttt{sum\_cap}. Each of these is monotone in both arguments and
continuous on $[0,1]^2$, and the candidate map \eqref{eq:candidate} is
monotone and continuous in $x$, so the round operator $G$, a finite
composition of these maps, is monotone and continuous on the complete
lattice $[0,1]^V$. The start vector $x_0$ (base facts) satisfies
$x_0 \le G(x_0)$, hence the trajectory
$x_0 \le G(x_0) \le G^2(x_0) \le \dots$ is pointwise non-decreasing and
bounded by $1$, so it converges to some $x^*$; continuity turns the
limit into a fixpoint, $G(x^*) = \lim G^{k+1}(x_0) = x^*$. Continuity
plus monotonicity make $G$ $\omega$-continuous on the chain from $x_0$,
so by Kleene's fixpoint theorem $x^*$ is the least fixpoint of $G$
above $x_0$. (Monotonicity alone would not suffice for the Kleene
argument; continuity is what licenses exchanging $G$ with the limit.)
\end{proposition}
...
Property-based tests corroborate the proposition, monotone
trajectory, boundedness, $\varepsilon$-fixpoint reached, and exact
agreement between the engine and a reference implementation of $G$,
on 1{,}050 random rule systems including seeded cycles, across the
three monotone aggregators. Randomized tests are evidence about the
implementation, not part of the proof; the proof is
Proposition~\ref{prop:convergence}, and the tests check that the code
matches it. Property testing also exposed a real defect: a
...
dependence determine the answer. The same coin has a second face:
under distribution semantics an additive per-rule decomposition of a
marginal is not uniquely determined without an additional attribution
rule. Overlapping proofs are reconciled by inclusion--exclusion, which
does not split into per-rule shares by itself; Shapley-style
attributions can be imposed on top, but each such rule imports
assumptions of its own.
\pla{}'s per-decision itemization exists \emph{because} its calculus
agrees to itemize, and that agreement is legitimate exactly when
supports are treated as independent. Users who need the
possible-world guarantee should use ProbLog~\cite{deraedt2007};
\pla{} trades it for hand-traceable, itemizable inference.
