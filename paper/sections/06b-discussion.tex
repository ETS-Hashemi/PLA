...
propositions capture those same tails. Where the signal is
tail-shaped, a small rule set loses little AUC (and a measured, bounded
slice of early precision) while buying full traceability. The RuleFit
comparison prices the threshold choice itself: mined continuous
thresholds recover early precision (AP $0.80$ vs.\ the static rules'
$0.67$) at a cost in AUC ($0.9437$ vs.\ $0.9660$, lower on all ten
splits), so even between rule models the choice tracks the deployment
metric rather than a single winner. The synthetic study shows the opposite regime: when the
...
multi-rule examples at the very top of the ranking, the region
...
The rule-learning baseline corroborates the diagnosis from outside
\pla{}: RuleFit, carrying its own mined pre-drift thresholds, degrades
the same way ($0.5825$ strict, $0.6065$ SOX) while the
continuous-feature models hold near $0.65$--$0.70$
...
measured in Section~\ref{sec:evaluation}, with a boundary the chained
study makes explicit: in the flat experiments the
exported ranking is the computation's own by algebra, for both
models; in chained programs the local candidate ordering is barely
better than chance at naming the most load-bearing rule, so there the
artifact to trust is the deletion counterfactual itself (the
leave-one-rule-out table the engine exports), not the raw candidate
magnitudes; whether such traces improve human audit decisions is the
