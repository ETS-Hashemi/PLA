...
that beats the constant-prevalence null baseline, rule-level traces
that beat reversed and permutation-averaged random controls on every
dataset --- and a pre-registered negative on the context-conditioning
mechanism itself: no practically useful improvement on the
pre-specified AUC endpoint under any of the three designs, with the
post-hoc credit-card average-precision gain held apart as a hypothesis
